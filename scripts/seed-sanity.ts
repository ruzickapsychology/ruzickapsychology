...
function paragraphsToPortableText(paragraphs: readonly string[], prefix: string) {
  return paragraphs.map((paragraph, index) => textBlock(paragraph, `${prefix}-${index}`))
}
...
async function buildDocuments(): Promise<SeedDocument[]> {
  const images = {
    ...
    homePortrait: await uploadImage('public/images/portrait.jpg', 'Dr. Christina Ruzicka'),
    portrait: await uploadImage('public/images/portrait-large.jpg', 'Dr. Christina Ruzicka'),
    ctaFloral: await uploadImage('public/images/cta-floral.jpg', 'Floral background'),
    contactRoses: await uploadImage('public/images/contact-roses.jpg', 'Soft pink roses'),
    imago: await uploadImage('public/images/imago-bg.jpg', 'Floral therapy background'),
    therapyRoom: await uploadImage('public/images/therapy-room.jpg', 'Therapy office seating area'),
    bouquet: await uploadImage('public/images/bouquet.jpg', 'Bouquet of flowers'),
  }
  ...
  const singletonDocs = [
    ...
    },
    {
      ...
      modality: {
        ...
        body: paragraphsToPortableText(specialties.modality.body, 'specialties-modality'),
...
async function main() {
  ...
  log(`[seed] Prepared ${documents.length} documents${dryRun ? ' (dry run)' : ''}.`)

  if (!dryRun) {
    for (const document of documents) {
      await client.createOrReplace(document)
      log(`[seed] Upserted ${document._id}`)
    }
  }
