...
- Primary CTA: `buttonClasses("primary")`, dark pill with light text from `src/components/ui/button.module.css`.
...
Avoid rogue button styles. If a new pill/button CTA style is needed, add it to `src/components/ui/button.module.css`, expose it through `buttonClasses()`, and document it here.
